...
static int virtio_pcie_init_pci(PCIDevice *pci_dev)
{
    VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);
    VirtIODevice *vdev;
    ...
    if (proxy->class_code != PCI_CLASS_STORAGE_SCSI &&
	proxy->class_code != PCI_CLASS_STORAGE_OTHER)
	proxy->class_code = PCI_CLASS_PCIE;
    error_report("iiiiii%d,%0x\n",(proxy->pcieconf).flag,proxy->class_code);
    vdev = virtio_pcie_init(&pci_dev->qdev);
    if (!vdev) {
	return -1;
    }
    error_report("work here 4!\n");
    vdev->nvectors = proxy->nvectors;
    error_report("work here 5!\n");
    virtio_init_pci(proxy, vdev);
    /* make the actual value visible */
    error_report("work here 6!\n");
    proxy->nvectors = vdev->nvectors;
    return 0;
}
...
static Property virtio_pcie_properties[] = {
    DEFINE_PROP_HEX32("class", VirtIOPCIProxy, class_code, 0),
 /*   DEFINE_BLOCK_PROPERTIES(VirtIOPCIProxy, blk.conf),
    DEFINE_BLOCK_CHS_PROPERTIES(VirtIOPCIProxy, blk.conf),
    DEFINE_PROP_STRING("serial", VirtIOPCIProxy, blk.serial),
...
{
    error_report("virtio_pcie_class_init excute\n");
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *k = PCI_DEVICE_CLASS(klass);

    k->init = virtio_pcie_init_pci;
    k->exit = virtio_blk_exit_pci;
    k->vendor_id = PCI_VENDOR_ID_REDHAT_QUMRANET;
    k->device_id = PCI_DEVICE_ID_VIRTIO_PCIE;
    k->revision = VIRTIO_PCI_ABI_VERSION;
    k->class_id = PCI_CLASS_PCIE;
    dc->reset = virtio_pci_reset;
    dc->props = virtio_pcie_properties;
}

static TypeInfo virtio_pcie_info = {
